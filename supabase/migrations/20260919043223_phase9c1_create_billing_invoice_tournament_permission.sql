...
begin
  ...
  if p_context_type = 'platform' then
    ...
    if v_account.tournament_id is null
       or not (public.is_org_admin(p_org_id)
               or public.has_tournament_permission('manage_tournament_finances', v_account.tournament_id)) then
      raise exception 'tournament finance permission required' using errcode = 'insufficient_privilege';
    end if;
  else
